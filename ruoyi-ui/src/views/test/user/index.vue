...
<script>
  ...
  export default {
      data () {
        console.log("目标上传url:"+process.env.VUE_APP_BASE_API+"/video/uploader/chunk");
          return {
              options: {
                
                  //目标上传 URL，默认POST
                  target: process.env.VUE_APP_BASE_API+"/video/uploader/chunk",
                  //分块大小(单位：字节)
                  chunkSize: '2048000',
      ...
      methods: {
          ...
          */
          ...
          computeMD5(file) {
              ...
              let currentChunk = 0;
              const chunkSize = 10 * 1024 * 1000;//9.5M
              let chunks = Math.ceil(file.size / chunkSize);
              console.log("分割成:"+chunks)
