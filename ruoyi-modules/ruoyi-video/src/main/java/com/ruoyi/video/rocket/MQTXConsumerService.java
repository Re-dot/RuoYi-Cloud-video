...
@Slf4j
@Component
@RocketMQMessageListener(topic = "RLT_TEST_TOPIC", selectorExpression = "charge", consumerGroup = "Con_Group_Four")
public class MQTXConsumerService implements RocketMQListener<UserCharge> {
    ...
    public void onMessage(UserCharge userCharge) {
        ...
        // 给用户增加积分
        int i = creditMapper.addNumber(userCharge.getUserId(), userCharge.getChargeAmount());//test
        if (1 == i) {
